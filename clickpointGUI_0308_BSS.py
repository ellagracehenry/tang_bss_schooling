...
ObjType = ["Head"]  # Default fish family
...
def load_video():
    ...
    slider_frame.configure(to=max(0, total_frames - 1))
...
def canvas_click_events(event):
    ...
    # --- NEW: Draw temporary circle on display frame ---
    ...
    # draw in display space, not original space
...
def edit_selected():
    ...
    objtype_entry = Entry(edit_win)
    objtype_entry.insert(0, annotation["ObjType"])
    objtype_entry.grid(row=3, column=1)
    ...
    def save_changes():
        try:
            ...
            annotation["ObjID"] = int(objid_entry.get())
            annotation["ObjType"] = objtype_entry.get()
...
def toggle_obj_type():
    if ObjType[0] == "Head":
        ObjType[0] = "Tail"
        button_toggle_obj_type.configure(text="Tail")
    else:
        ObjType[0] = "Head"
        button_toggle_obj_type.configure(text="Head")
...
def save_annotations():
    ...
    if save_locations_var.get():
        ...
        csv_path = os.path.join(save_dir, f"{file_name}_annotations.csv")
...
button_browse = ctk.CTkButton(frame_controls, text="Browse Images", command=load_video, height = 20)
...
def on_mouse_wheel(event):
    ...
    zoom_step = 50  # pixels per scroll tick (adjust for sensitivity)
    ...
    if hasattr(event, "delta"):
        if event.delta > 0:
            ...
            video_size_x = max(100, video_size_x - zoom_step)  # zoom out
    ...
    # Optionally, update the slider to reflect current zoom
    slider_zoom.set(video_size_x)

# Windows and Linux
canvas_video.bind("<MouseWheel>", on_mouse_wheel)  
# macOS (different events)
...
canvas_video.bind_all("<Up>", pan_canvas)
canvas_video.bind_all("<Down>", pan_canvas)
canvas_video.bind_all("<Left>", pan_canvas)
canvas_video.bind_all("<Right>", pan_canvas)
